...
namespace ContactsUploader
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var users = LoadUsersFromCsv(@"c:\temp\ADNamesToInsert.csv");
            var userJson = JsonConvert.SerializeObject(users.FirstOrDefault());

            client.CreateUser(userJson).Wait();
        ...
        private static IEnumerable<GraphUserModel> LoadUsersFromCsv(string path)
        {
            List<GraphUserModel> results = new List<GraphUserModel>();
            string[] colNames;
            bool wasHeaderRead = false;

            using (var reader = new StreamReader(@"c:\temp\ADNamesToInsert.csv"))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    results.Add(new GraphUserModel
                    {
                        accountEnabled = true,
                        signInNames = new SignInName[] { new SignInName("emailAddress", cols[1]) },
                        creationType = cols[2],
                        displayName = cols[3],
                        mailNickname = cols[4],
                        passwordProfile = new Passwordprofile(cols[5], false),
                        passwordPolicies = cols[7]
                    });
                }
            }
            return results;
